...
CREATE TABLE IF NOT EXISTS app_users (
  ...
  roles         TEXT[] NOT NULL DEFAULT ARRAY['super_admin'],
  permissions   TEXT[] NOT NULL DEFAULT ARRAY['analytics:read', 'anomalies:read', 'members:read', 'checkins:read', 'payments:read'],
...
CREATE TABLE IF NOT EXISTS members (
  ...
  name            TEXT NOT NULL,
  email           TEXT UNIQUE,
  phone           TEXT CHECK (phone IS NULL OR phone ~ '^[789][0-9]{9}$'),
...
CREATE TABLE IF NOT EXISTS checkins (
  ...
  CHECK (checked_out IS NULL OR checked_out > checked_in)
);
